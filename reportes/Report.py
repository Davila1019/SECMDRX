...
class Report(FPDF):
    ...
    def header(self):
        ...
        self.image('logo-ipn.png', 1.5, .7, 3)
        self.image('logo-upiiz.png', 17.3, .4, 2.3)
    ...
    def create_pdf(self,metal,distancias:list,intensidad:list,angulos:list,indices:list,radioA=0,parametro=0):
        ...
        self.cell(0,.8,'Reporte simulación',0,0,'J')
        self.ln(1.2)
        ...
        while i < len(distancias):
            self.cell(15, .8,i.__str__()+'          '+str(indices[i])+'           '+str(distancias[i])+'          '+str(angulos[i]), 1, 0, 'J')
            self.ln(0.8)
            i += 1
        self.set_font('Times', 'B', 12)
